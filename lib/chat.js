...
You have ${hasStudyMemory ? "seven tools" : "five tools"}:
...
const TOOLS = [GATHER_TOOL, SEARCH_LEXICON_TOOL, FIND_OCCURRENCES_TOOL, SEARCH_BIBLE_TEXT_TOOL, FIND_CROSS_REFERENCES_TOOL];
...
// Builds the tools array for one turn — the base five tools always,
...
// text for its tool_result, plus any gatherPassage() data or cross-
// reference diagram it produced (only gather_passage/find_cross_references
// produce these; the other tools are text-only research aids with nothing
// structured to show the frontend).
async function runTool(toolUse, { appKey, userId, isPaid }) {
  switch (toolUse.name) {
    case "gather_passage": {
      const { text, gathered } = await runGatherTool(toolUse.input, { appKey });
      return { text, gathered, diagram: null };
    }
    case "search_lexicon":
      return { text: await runSearchLexiconTool(toolUse.input), gathered: null, diagram: null };
    case "find_occurrences":
      return { text: await runFindOccurrencesTool(toolUse.input), gathered: null, diagram: null };
    case "search_bible_text":
      return { text: await runSearchBibleTextTool(toolUse.input), gathered: null, diagram: null };
    case "find_cross_references": {
      const { text, diagram } = await runFindCrossReferencesTool(toolUse.input);
      return { text, gathered: null, diagram };
    }
    case "search_study_history":
      return { text: await runSearchStudyHistoryTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null };
    case "search_my_notes":
      return { text: await runSearchMyNotesTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null };
    default:
      return { text: `Error: unknown tool "${toolUse.name}".`, gathered: null, diagram: null };
  }
}
 ...
 * conversationId is null for an anonymous request (userId omitted). gathered
 * is the list of gatherPassage() results (translations/original-language/
 * commentary) for every distinct reference Claude looked up while producing
 * this reply — so the frontend can show the same source material Claude
 * actually used, not just the prose answer. crossReferences is the list of
 * find_cross_references() results (one entry per distinct focus verse
 * Claude looked up), each { reference, results, totalCount } — the
 * frontend draws these as a diagram, same "same data Claude saw" reasoning
 * as gathered. (search_lexicon/find_occurrences/search_study_history
 * results are text-only and only ever seen by Claude, not surfaced
 * separately to the frontend — Claude is expected to fold anything
 * relevant into its reply.)
 *
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (toolUse) => {
        const { text, gathered, diagram } = await runTool(toolUse, { appKey, userId, isPaid });
