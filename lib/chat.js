...
import { appendToConversation, getPaidProfile, logStudyEntry, searchStudyHistory } from "./supabase.js";
...
function buildSystemPrompt(hasUser, isPaid, agentName = null) {
  ...
  const studyMemoryUpsellLine =
    hasUser && !isPaid
      ? `\n\nThis user is signed in but on the free tier, so you do NOT have search_study_history (recalling their past study across conversations is a Pro feature). If they ask you to recall something from a past conversation, let them know that's a Pro feature they can turn on from the Subscription page, rather than guessing or pretending you checked.`
      : "";
...
You have ${hasStudyMemory ? "five tools" : "four tools"}:
...
// Builds the tools array for one turn — the base four tools always,
// search_study_history only for a signed-in, paid user (see the
// SEARCH_STUDY_HISTORY_TOOL comment above).
function buildTools(hasUser, isPaid) {
  return hasUser && isPaid ? [...TOOLS, SEARCH_STUDY_HISTORY_TOOL] : TOOLS;
}
...
async function runTool(toolUse, { appKey, userId, isPaid }) {
  switch (toolUse.name) {
    ...
    case "search_study_history":
      return { text: await runSearchStudyHistoryTool(toolUse.input, { userId, isPaid }), gathered: null };
    default:
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (toolUse) => {
        const { text, gathered } = await runTool(toolUse, { appKey, userId, isPaid });
