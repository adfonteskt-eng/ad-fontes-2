...
You have ${hasStudyMemory ? "nine tools" : "seven tools"}:
...
async function runTool(toolUse, { appKey, userId, isPaid }) {
  switch (toolUse.name) {
    case "gather_passage": {
      ...
      return { text, gathered, diagram: null, map: null, briefing: null };
    }
    case "search_lexicon":
      return { text: await runSearchLexiconTool(toolUse.input), gathered: null, diagram: null, map: null, briefing: null };
    case "find_occurrences":
      return { text: await runFindOccurrencesTool(toolUse.input), gathered: null, diagram: null, map: null, briefing: null };
    case "search_bible_text":
      return { text: await runSearchBibleTextTool(toolUse.input), gathered: null, diagram: null, map: null, briefing: null };
    case "find_cross_references": {
      const { text, diagram } = await runFindCrossReferencesTool(toolUse.input);
      return { text, gathered: null, diagram, map: null, briefing: null };
    }
    case "generate_map": {
      const { text, map } = await runGenerateMapTool(toolUse.input);
      return { text, gathered: null, diagram: null, map, briefing: null };
    }
    case "generate_passage_briefing": {
      const { text, briefing } = await runPassageBriefingTool(toolUse.input);
      return { text, gathered: null, diagram: null, map: null, briefing };
    }
    case "search_study_history":
      return { text: await runSearchStudyHistoryTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null, map: null, briefing: null };
    case "search_my_notes":
      return { text: await runSearchMyNotesTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null, map: null, briefing: null };
    default:
      return { text: `Error: unknown tool "${toolUse.name}".`, gathered: null, diagram: null, map: null, briefing: null };
  }
}
 ...
 * maps, briefings, quoteVerification } — conversationId is null for an
 * anonymous request (userId omitted). gathered is the list of
 ...
 * Claude looked up), each { reference, results, totalCount }. maps is the
 ...
 * quoteVerification is verifyReplyQuotes()'s result (see lib/verify.js):
 * every span of `reply` presented as a direct Scripture quotation, checked
 * against the real translation text in `gathered` — Receipts Mode's
 * backstop against a plausible-sounding but wrong quotation. All five
 * drive frontend visuals from the same data Claude actually saw, not a
 * separately-fetched copy. (search_lexicon/find_occurrences/
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (toolUse) => {
        const { text, gathered, diagram, map, briefing } = await runTool(toolUse, { appKey, userId, isPaid });
