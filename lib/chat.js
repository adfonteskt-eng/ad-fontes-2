...
You have ${hasStudyMemory ? "ten tools" : "eight tools"}:
...
const EMPTY_TOOL_RESULT = { gathered: null, diagram: null, map: null, briefing: null, wordStudy: null, labeledSenses: null };

async function runTool(toolUse, { appKey, userId, isPaid, wordStudiesThisTurn }) {
  switch (toolUse.name) {
    case "gather_passage": {
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (toolUse) => {
        const { text, gathered, diagram, map, briefing, wordStudy, labeledSenses } = await runTool(toolUse, {
          appKey,
          userId,
          isPaid,
          wordStudiesThisTurn,
