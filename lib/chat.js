...
function buildSystemPrompt(hasUser, isPaid, agentName = null, depthLevel = DEFAULT_DEPTH_LEVEL) {
...
You have ${hasStudyMemory ? "eight tools" : "six tools"}:
...
Write like you're talking, not filing a report: prose, not section headers or bullet-point dumps, unless the user's question specifically calls for a list, or is a sermon/lesson/discussion-guide request from a paid account (see above), which gets the structured format described above instead. Keep replies focused — a few solid paragraphs at most, more only if the question genuinely needs it.
...
const TOOLS = [GATHER_TOOL, SEARCH_LEXICON_TOOL, FIND_OCCURRENCES_TOOL, SEARCH_BIBLE_TEXT_TOOL, FIND_CROSS_REFERENCES_TOOL, GENERATE_MAP_TOOL];
...
async function runFindCrossReferencesTool(input) {
  ...
  try {
    ...
    return {
      ...
      diagram: { reference, results, totalCount },
    };
  } catch (error) {
...
async function runTool(toolUse, { appKey, userId, isPaid }) {
  switch (toolUse.name) {
    case "gather_passage": {
      const { text, gathered } = await runGatherTool(toolUse.input, { appKey });
      return { text, gathered, diagram: null, map: null };
    }
    case "search_lexicon":
      return { text: await runSearchLexiconTool(toolUse.input), gathered: null, diagram: null, map: null };
    case "find_occurrences":
      return { text: await runFindOccurrencesTool(toolUse.input), gathered: null, diagram: null, map: null };
    case "search_bible_text":
      return { text: await runSearchBibleTextTool(toolUse.input), gathered: null, diagram: null, map: null };
    case "find_cross_references": {
      const { text, diagram } = await runFindCrossReferencesTool(toolUse.input);
      return { text, gathered: null, diagram, map: null };
    }
    case "generate_map": {
      const { text, map } = await runGenerateMapTool(toolUse.input);
      return { text, gathered: null, diagram: null, map };
    }
    case "search_study_history":
      return { text: await runSearchStudyHistoryTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null, map: null };
    case "search_my_notes":
      return { text: await runSearchMyNotesTool(toolUse.input, { userId, isPaid }), gathered: null, diagram: null, map: null };
    default:
      return { text: `Error: unknown tool "${toolUse.name}".`, gathered: null, diagram: null, map: null };
  }
}
 ...
 * maps, quoteVerification } — conversationId is null for an anonymous
 * request (userId omitted). gathered is the list of gatherPassage()
 * results (translations/original-language/commentary) for every distinct
 * reference Claude looked up while producing this reply — so the frontend
 * can show the same source material Claude actually used, not just the
 * prose answer. crossReferences is the list of find_cross_references()
 * results (one entry per distinct focus verse Claude looked up), each
 * { reference, results, totalCount }. maps is the list of generate_map()
 * results (one per call), each { title, route, waypoints, extraPoints,
 * unresolved } — see lib/geography.js. quoteVerification is
 * verifyReplyQuotes()'s result (see lib/verify.js): every span of
 * `reply` presented as a direct Scripture quotation, checked against the
 * real translation text in `gathered` — Receipts Mode's backstop against a
 * plausible-sounding but wrong quotation. All four drive frontend visuals
 * from the same data Claude actually saw, not a separately-fetched copy.
 * (search_lexicon/find_occurrences/search_study_history results are
...
export async function chatTurn({
  ...
  const { isPaid, agentName: rawAgentName, depthLevel: storedDepthLevel } = hasUser
  ...
  const system = buildSystemPrompt(hasUser, isPaid, agentName, effectiveDepthLevel);
  const tools = buildTools(hasUser, isPaid);

  let finalReply = null;
  const gatheredThisTurn = [];
  const seenRefs = new Set();
  const crossReferencesThisTurn = [];
  const seenCrossRefFocus = new Set();
  const mapsThisTurn = [];
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (toolUse) => {
        const { text, gathered, diagram, map } = await runTool(toolUse, { appKey, userId, isPaid });
        if (gathered && !seenRefs.has(gathered.reference.usfm)) {
          seenRefs.add(gathered.reference.usfm);
          gatheredThisTurn.push(gathered);
        }
        if (diagram && !seenCrossRefFocus.has(diagram.reference)) {
          seenCrossRefFocus.add(diagram.reference);
          crossReferencesThisTurn.push(diagram);
        }
    ...
    appendToConversation(userId, resolvedConversationId, {
      ...
      entries: [
        { role: "user", text: message.trim() },
        {
          ...
          crossReferences: crossReferencesThisTurn.length > 0 ? crossReferencesThisTurn : null,
  ...
  return {
    ...
    crossReferences: crossReferencesThisTurn,
    quoteVerification,
    maps: mapsThisTurn,
