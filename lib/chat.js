...
import { logStudyEntry, searchStudyHistory } from "./supabase.js";
...
function buildSystemPrompt(hasUser) {
  return `You are a warm, knowledgeable conversation partner for Bible study — closer to a thoughtful theologian or Bible scholar talking with someone than a search engine or report generator.

You have ${hasUser ? "four tools" : "three tools"}:
 ...
 * not just the prose answer. (search_lexicon/find_occurrences/
 ...
 * study memory search_study_history later searches. Entirely additive:
...
export async function chatTurn({
  ...
  const system = buildSystemPrompt(hasUser);
