 ...
 * maps } — conversationId is null for an anonymous request (userId
 * omitted). gathered is the list of gatherPassage() results (translations/
 * original-language/commentary) for every distinct reference Claude looked
 * up while producing this reply — so the frontend can show the same source
 * material Claude actually used, not just the prose answer.
 * crossReferences is the list of find_cross_references() results (one
 * entry per distinct focus verse Claude looked up), each { reference,
 * results, totalCount }. maps is the list of generate_map() results (one
 * per call), each { title, route, waypoints, extraPoints, unresolved } —
 * see lib/geography.js. All three drive frontend visuals from the same
 * data Claude actually saw, not a separately-fetched copy. (search_lexicon/
 * find_occurrences/search_study_history results are text-only and only
 * ever seen by Claude, not surfaced separately to the frontend — Claude is
