...
// Study Trail (spec item 11): a distilled "what did I actually study, in
// what order" digest of a conversation, distinct from the full transcript
// conversationExportModel() above. Deliberately not a new recording
// mechanism -- every structured artifact a study session touches
// (gathered passages, cross-references, maps, passage briefings, word
// studies) is already persisted per-turn on the conversation's own
// render_log (see lib/chat.js's chatTurn() and lib/supabase.js's
// appendToConversation()), so the "trail" is just that same data walked
// in order and deduplicated by each artifact's own natural key, first
// occurrence wins. No new table, no new write path.
function trailStopKey(type, item) {
  if (type === "passage") return item.reference?.usfm;
  if (type === "crossReferences") return item.reference;
  if (type === "map") return item.title;
  if (type === "briefing") return item.reference;
  if (type === "wordStudy") return item.strongsNumber;
  return null;
}

function trailStopText(type, item) {
  if (type === "passage") return item.reference.usfm;
  if (type === "crossReferences") return `${item.reference} — ${item.totalCount} connection${item.totalCount === 1 ? "" : "s"} (curated scholarly dataset)`;
  if (type === "map") return item.title;
  if (type === "briefing") return `${item.reference} — ${item.genre}`;
  if (type === "wordStudy") return `${item.strongsNumber} — ${item.totalCount} occurrence${item.totalCount === 1 ? "" : "s"}`;
  return "";
}

const TRAIL_STOP_LABELS = {
  passage: "Passage studied",
...
export function studyTrailExportModel(conversation) {
  const meta = [];
  const updated = formatDate(conversation.updatedAt ?? conversation.updated_at);
  if (updated) meta.push(`Last updated ${updated}`);
  ...
  const seen = new Set();
  const blocks = [];
  const pushStops = (type, items) => {
    for (const item of items ?? []) {
      const key = `${type}:${trailStopKey(type, item)}`;
      if (!trailStopKey(type, item) || seen.has(key)) continue;
      seen.add(key);
      blocks.push({ label: TRAIL_STOP_LABELS[type], text: trailStopText(type, item) });
    }
  };

  for (const entry of conversation.renderLog ?? conversation.render_log ?? []) {
    if (entry.role !== "assistant") continue;
    // Briefing before passage, matching the same orientation-first
    // ordering public/app.js already renders a turn's cards in.
    pushStops("briefing", entry.briefings);
    pushStops("passage", entry.gathered);
    pushStops("crossReferences", entry.crossReferences);
    pushStops("map", entry.maps);
    pushStops("wordStudy", entry.wordStudies);
  }

  return {
    title: conversation.title ? `Study Trail — ${conversation.title}` : "Study Trail",
    meta,
    blocks:
      blocks.length > 0
        ? blocks
        : [{ text: "Nothing to show yet — a trail fills in as you study passages, look up cross-references, run a word study, or check a map or briefing." }],
  };
}
