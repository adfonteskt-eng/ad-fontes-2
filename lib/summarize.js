...
// Same reasoning as lib/chat.js's ANTHROPIC_TIMEOUT_MS: generous, since a
// real summary completion can legitimately take a few seconds, but finite
// so a stalled request fails cleanly instead of hanging the CLI or the
// GET /api/passage endpoint forever.
...
/**
 * Flattens a gatherPassage() result into the same plain-text block the
 * one-shot summary prompt uses: reference, translations, original-language
 * word-by-word, commentary excerpts. Exported so lib/chat.js can hand
 * Claude the identical material as a tool result when it calls
 * gather_passage mid-conversation — one formatter, two callers.
 */
export function formatGatheredPassage(gathered) {
  const sections = [
...
export async function summarizePassage(
  ...
  const prompt = formatGatheredPassage(gathered);

  // Deliberately not using prompt caching here, unlike lib/chat.js —
  // checked, not assumed: this SYSTEM_PROMPT alone is only ~263 tokens,
  // well under Claude Sonnet 5's 1,024-token cache minimum, and there's no
  // tools array or growing history to combine it with (this is a one-shot
  // call, not a conversation). Caching would silently do nothing here but
  // add complexity. Revisit if SYSTEM_PROMPT grows substantially.
