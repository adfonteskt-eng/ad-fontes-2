 ...
 * both server.js's GET /api/preferences (the frontend needs both to decide
 * whether to show the name-your-agent field vs. its upsell -- see
 * public/auth.js's loadPreferences()) and getAgentNameIfPaid() below (chat's
 * own per-turn lookup). Defaults to { isPaid: false, agentName: null } if
...
/**
 * Convenience for lib/chat.js's per-turn system-prompt build: the agent's
 * chosen display name, but only if this account is actually paid (a
 * downgraded account's old agent_name value stays in the column but should
 * stop being used the moment is_paid flips back to false, without a
 * separate migration to blank it out). Returns null for an anonymous
 * request, an unpaid account, or an empty/whitespace-only name.
 */
export async function getAgentNameIfPaid(userId) {
  if (!userId) return null;
  const { isPaid, agentName } = await getPaidProfile(userId);
