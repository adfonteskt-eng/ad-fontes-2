...
// Depth slider (free for every user, unlike agent_name/reading-plan
// reminders below -- see lib/chat.js's buildSystemPrompt()): how technical
// an answer should be. Kept here, not in lib/chat.js, so both server.js
// (validating a client-supplied override) and lib/chat.js (building the
// system prompt) import the same list without a circular import between
// the two (lib/chat.js already imports getPaidProfile from this file).
// Mirrors the three check-constraint values in supabase/schema.sql by
// hand, since Postgres can't reference an app-side constant.
export const DEPTH_LEVELS = ["everyday", "student", "scholar"];
export const DEFAULT_DEPTH_LEVEL = "everyday";

export function isValidDepthLevel(level) {
  return DEPTH_LEVELS.includes(level);
}

// Tradition Lens (free for every user, same as depth level above -- see
// lib/chat.js's buildSystemPrompt()): an optional "home tradition" so
// Claude can note where the user's own tradition stands when a passage is
// genuinely debated. Unlike depth level, null is a real, common value
// ("prefer not to say" / never set) -- there's no reasonable universal
// default the way "everyday" is for depth. Mirrors the check-constraint
// list in supabase/schema.sql by hand, same reasoning as DEPTH_LEVELS.
export const HOME_TRADITIONS = [
  "reformed",
  "baptist",
  "wesleyan",
  "lutheran",
...
export function isValidHomeTradition(tradition) {
  return tradition === null || HOME_TRADITIONS.includes(tradition);
}

/**
 * Returns { isPaid, agentName, depthLevel, homeTradition } for a signed-in
 * user in one query -- backs server.js's GET /api/preferences (the
 * frontend needs all four: isPaid/agentName to decide whether to show the
 * name-your-agent field vs. its upsell -- see public/auth.js's
 * loadPreferences() -- depthLevel to pre-select the depth slider, and
 * homeTradition to pre-select the tradition dropdown) and lib/chat.js's
 * own per-turn checks (the paid-gated tools/features, this user's stored
 * default depth, and their home tradition for Tradition Lens). Defaults to
 * { isPaid: false, agentName: null, depthLevel: DEFAULT_DEPTH_LEVEL,
 * homeTradition: null } if Supabase isn't configured or there's somehow no
 * profiles row yet.
 */
export async function getPaidProfile(userId) {
  if (!isSupabaseConfigured()) {
    return { isPaid: false, agentName: null, depthLevel: DEFAULT_DEPTH_LEVEL, homeTradition: null };
  }
  const rows = await postgrest(`profiles?id=eq.${userId}&select=is_paid,agent_name,depth_level,home_tradition`, { method: "GET" });
  const row = rows?.[0];
  return {
    isPaid: Boolean(row?.is_paid),
    agentName: row?.agent_name ?? null,
    depthLevel: isValidDepthLevel(row?.depth_level) ? row.depth_level : DEFAULT_DEPTH_LEVEL,
    homeTradition: isValidHomeTradition(row?.home_tradition ?? null) ? row?.home_tradition ?? null : null,
  };
}

/**
 * Sets a signed-in user's default depth level (free feature -- server.js's
 * handleSetPreferences validates the value against DEPTH_LEVELS before
 * calling this, same "validate at the edge" split as setAgentName below).
 */
export async function setDepthLevel(userId, level) {
  await postgrest(`profiles?id=eq.${userId}`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({ depth_level: level }),
  });
}

/**
 * Sets a signed-in user's home tradition (free feature, same validate-at-
 * the-edge split as setDepthLevel -- server.js checks isValidHomeTradition
 * first). `tradition: null` clears it back to "prefer not to say."
 */
export async function setHomeTradition(userId, tradition) {
  await postgrest(`profiles?id=eq.${userId}`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({ home_tradition: tradition || null }),
  });
}
...
// --- Stripe billing (profiles.stripe_customer_id / .stripe_subscription_id
// / .is_paid) --------------------------------------------------------------
// See lib/stripe.js's header comment for the overall flow. is_paid used to
// be hand-set in the Supabase dashboard (see README -> Subscription / paid
// tier); it's now driven by server.js's webhook handler instead, via these
// two functions. Not fire-and-forget, same reasoning as setDigestOptIn --
// a webhook the caller (server.js) doesn't get a real error back from is a
// webhook Stripe will just keep retrying against a profile that never
// actually updated.

/**
 * Looks up a profile by its Stripe customer id -- used when a webhook event
 * doesn't carry the app's own user id in metadata (see server.js's webhook
 * handler). Returns { id } or null.
 */
...
/**
 * Upserts a user's Stripe billing state onto their profiles row --
 * `stripeCustomerId`/`stripeSubscriptionId` default to leaving the existing
 * value alone (pass explicit `null` to clear one, e.g. once a cancelled
 * subscription's grace period is over -- server.js doesn't currently do
 * this, but the option exists rather than being impossible), while `isPaid`
 * is always set explicitly by the caller (there's no sensible "leave alone"
 * default for the one field every other paid-feature check reads).
 */
export async function setBillingProfile(userId, { stripeCustomerId, stripeSubscriptionId, isPaid }) {
  const patch = { is_paid: Boolean(isPaid) };
...
/**
 * Returns a signed-in user's stripe_customer_id, or null -- used by
 * server.js's checkout/portal routes to reuse an existing Stripe customer
 * (checkout) or require one (portal, which can't create a customer itself).
 */
export async function getStripeCustomerId(userId) {
  if (!isSupabaseConfigured()) return null;
  const rows = await postgrest(`profiles?id=eq.${userId}&select=stripe_customer_id`, { method: "GET" });
