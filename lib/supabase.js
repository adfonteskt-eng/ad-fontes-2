...
/**
 * Returns { isPaid, agentName, depthLevel } for a signed-in user in one
 * query -- backs server.js's GET /api/preferences (the frontend needs all
 * three: isPaid/agentName to decide whether to show the name-your-agent
 * field vs. its upsell -- see public/auth.js's loadPreferences() -- and
 * depthLevel to pre-select the depth slider) and lib/chat.js's own per-turn
 * checks (the paid-gated tools/features, and this user's stored default
 * depth when a message doesn't send an explicit override). Defaults to
 * { isPaid: false, agentName: null, depthLevel: DEFAULT_DEPTH_LEVEL } if
 * Supabase isn't configured or there's somehow no profiles row yet.
 */
export async function getPaidProfile(userId) {
  if (!isSupabaseConfigured()) return { isPaid: false, agentName: null, depthLevel: DEFAULT_DEPTH_LEVEL };
  const rows = await postgrest(`profiles?id=eq.${userId}&select=is_paid,agent_name,depth_level`, { method: "GET" });
  const row = rows?.[0];
  return {
