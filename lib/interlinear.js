...
import { bookName, bookOrder } from "./bible-books.js";
...
// Manuscript variant "how much this matters" layer (spec item 9). The
// witness marker's own meaning (N/K/O, case, parentheses) is officially
// documented by STEPBible themselves right in TAGNT-Mat-Jhn.txt's header
// (same CC BY 4.0 file this project already reads), including a worked
// table of exactly these five significance bands with real counts across
// the whole NT — this isn't a scheme invented here, it's STEPBible's own
// categorization, adapted into short plain-English notes. Quoting the
// relevant lines (STEPBible-Data, CC BY 4.0, github.com/STEPBible):
//
//   NKO / NK(O) / NK(o)   -- identical in virtually all manuscripts (94%
//     of NT words); N(K)(O) / N(k)(o) -- different in Traditional
//     manuscripts, might translate differently; K(O) / k(o) -- found in
//     Traditional but not Ancient manuscripts, translated in KJV-style
//     Bibles; N(O) / n(o) -- found in Ancient but not Traditional
//     manuscripts, translated in modern Bibles; O / o -- different from
//     both Ancient and Traditional, rarely translated in any Bible.
//   N = Nestlé-Aland (Ancient), the text most modern translations follow.
//   K = the Textus Receptus behind the KJV ("Traditional").
//   Brackets mark a variant reading; lowercase marks a difference too
//   minor to necessarily affect translation.
//
// The five patterns above are illustrative, not exhaustive (STEPBible's
// own header says so with a trailing "..." on one row) -- the real data
// has ~20 distinct literal patterns (confirmed by scanning the actual
// file). VARIANT_SIGNIFICANCE below classifies by the general rule those
// five examples embody (whether N and/or K appear *bare* — capitalized
// and unparenthesized — vs. only inside parentheses/lowercase vs. absent
// entirely), which correctly reduces every real pattern in the dataset to
// one of these five bands; see classifyVariantSignificance()'s tests for
// the full real-pattern coverage this was checked against.
export const VARIANT_SIGNIFICANCE = {
  agreement: {
    label: "Agreement across editions",
    note: "This reading is essentially the same across the Ancient (Nestlé-Aland), Traditional (KJV/Textus Receptus), and other major Greek editions — true of about 94% of the New Testament's words. Nothing to reconcile here.",
  },
  ancientDiffersFromTraditional: {
    label: "Ancient text differs from the Traditional (KJV) text",
    note: "The Ancient Greek text (Nestlé-Aland) that most modern translations follow reads differently here than the Traditional text (Textus Receptus) behind the King James Version — the single most common reason a modern translation and the KJV word this differently.",
  },
  traditionalOnly: {
    label: "In the Traditional (KJV) text, not the Ancient text",
    note: "This word is in the Textus Receptus behind the King James Version, but not in the Ancient Greek text (Nestlé-Aland) most modern translations are based on — expect it in the KJV/NKJV but not in translations like the ESV, NIV, or NASB.",
  },
  ancientOnly: {
    label: "In the Ancient text, not the Traditional (KJV) text",
    note: "This word is in the Ancient Greek text (Nestlé-Aland) most modern translations are based on, but not in the Textus Receptus behind the King James Version — expect it in translations like the ESV, NIV, or NASB but not in the KJV/NKJV.",
  },
  otherOnly: {
    label: "Found only in other editions",
    note: "This reading matches neither the Ancient (Nestlé-Aland) nor Traditional (KJV/Textus Receptus) mainstream text — a rarer variant found in other Greek editions or manuscripts, seldom followed by any major translation.",
  },
};

// A letter is "bare" (unparenthesized) if it survives stripping every
// "(...)" group — that's what distinguishes "this edition's *main*
// reading" from "present only as a flagged variant," per the header
// comment above.
function stripParentheticals(witnesses) {
  return witnesses.replace(/\([^)]*\)/g, "");
}

/**
 * Classifies a TAGNT witness marker (e.g. "NKO", "N(k)O", "ko") into one
 * of the five significance bands STEPBible's own TAGNT header documents
 * — see VARIANT_SIGNIFICANCE above for the real source text. Returns one
 * of that object's keys.
 */
export function classifyVariantSignificance(witnesses) {
  const bare = stripParentheticals(witnesses ?? "");
  const bareHasN = bare.includes("N");
  const bareHasK = bare.includes("K");
  const anyN = /[Nn]/.test(witnesses ?? "");
  const anyK = /[Kk]/.test(witnesses ?? "");

  if (bareHasN && bareHasK) return "agreement";
  if (bareHasN && anyK) return "ancientDiffersFromTraditional";
  if (anyK && !anyN) return "traditionalOnly";
  if (anyN && !anyK) return "ancientOnly";
  return "otherOnly";
}
 ...
 *
 * Returns { occurrences, totalCount, byBook, error }. occurrences is
 * capped at `limit` (default 20); totalCount is the real total even when
 * truncated, so a very common word (e.g. Greek "kai", G2532, "and") is
 * honestly reported as extremely common rather than silently cut down to
 * 20 results with no indication there's more. byBook (Word-Study Web,
 * spec item 6) is a real per-book frequency tally computed from every
 * occurrence, not just the capped subset — see tallyOccurrencesByBook().
 */
...
// Real, dataset-backed frequency-by-book tally (Word-Study Web, spec item
// 6) — computed from the FULL hit list before it's capped to `limit`, so
// a book with occurrences past the cap still counts correctly. Unlike
// "sense clustering" (an interpretive judgment call the tagged data
// doesn't encode), a raw count of which books a word actually appears in
// is a plain fact about the dataset, no model involved. Sorted in
// canonical Bible order (Genesis -> Revelation), not by count, since
// "where does this cluster across the canon" reads more naturally in
// reading order than a leaderboard.
function tallyOccurrencesByBook(hits) {
  const counts = new Map();
  for (const hit of hits) {
    const book = hit.reference.split(".")[0];
    counts.set(book, (counts.get(book) ?? 0) + 1);
  }
  return [...counts.entries()]
    .map(([book, count]) => ({ book, name: bookName(book) ?? book, count }))
...
async function findStrongsOccurrencesUncached(target, strongsNumber, limit) {
  ...
  return { occurrences: hits.slice(0, limit), totalCount: hits.length, byBook: tallyOccurrencesByBook(hits), error: null };
}
