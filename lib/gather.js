...
async function gatherGreek(reference, { includeVariants }) {
  ...
  if (allWords.length === 0) {
    return {
      ...
      variantCount: 0,
      missingFiles: [],
      error: `No tagged Greek found for ${reference.key}.`,
      source: null,
    };
  }
  ...
  return {
    ...
    missingFiles: [],
    error: null,
    source: "STEPBible / Tyndale House Cambridge (CC BY 4.0) — NA28 critical text",
  };
}
 ...
 * concurrently. Nothing here prints anything; see index.js for that.
 */
export async function gatherPassage(
  usfm,
  { appKey, includeVariants = false, translations } = {},
) {
